...
use crate::database::find_project_root;
...
impl Config {
    pub fn read() -> Result<Self, Box<dyn Error + Send + Sync + 'static>> {
        let path = find_project_root().unwrap_or_default().join("diesel.toml");
